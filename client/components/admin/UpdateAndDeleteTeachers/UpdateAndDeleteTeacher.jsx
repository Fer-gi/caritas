...
import {
  ...
  updateTeacherType,
  deleteTeacher,
} from "../../../../server/firebase/controllers/admin/updateanddeleteteacher/updateanddeleteteacher";
  ...
  const handleSaveClick = async (userId) => {
    ...
    const confirmation = window.confirm(
      `¿Estás seguro de que quieres actualizar a ${
        teachers.find((teacher) => teacher.id === userId).username
      } a ${selectedType}?`
    );
        ...
        <Card key={teacher.id} style={{ margin: "10px" }}>
          <Card.Body>
            ...
            <Form.Select onChange={handleSelectChange}>
              <option value="student" selected={teacher.type === "student"}>
                Alumno
              </option>
              <option value="teacher" selected={teacher.type === "teacher"}>
                Profesor
              </option>
              <option value="admin" selected={teacher.type === "admin"}>
                Admin
              </option>
            </Form.Select>
            <Button
